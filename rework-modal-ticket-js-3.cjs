...
const startIdx = js.indexOf('function activarBotonesVerTicket() {');
const endString = 'console.error("Error obteniendo ticket:", error);\n        }\n      });\n    });\n  }';
const endIdx = js.indexOf(endString) + endString.length;

if (startIdx !== -1 && js.indexOf(endString) !== -1) {
  ...
  console.log("Updated via substring logic");
} else {
