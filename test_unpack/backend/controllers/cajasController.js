...
export const getCajaHoy = async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      `SELECT * FROM cajas 
       WHERE comercio_id = $1 
       AND (estado = 'abierta' OR fecha = COALESCE($2, date('now', 'localtime')))
  ...
  try {
    ...
    const { rows } = await pool.query(
      `INSERT INTO cajas (comercio_id, fecha, saldo_inicial)
       VALUES ($1, COALESCE($3, date('now', 'localtime')), $2)
       RETURNING *`,
  ...
  try {
    ...
    const cajaQuery = await pool.query(
      `SELECT hora_apertura, hora_cierre FROM cajas 
       WHERE comercio_id = $1 
       AND (estado = 'abierta' OR fecha = date('now', 'localtime'))
       ORDER BY hora_apertura DESC LIMIT 1`,
      [comercioId]
    );
    ...
    const timeCondition = endTime ? `AND fecha >= $2 AND fecha <= $3` : `AND fecha >= $2 AND date(fecha) <= date('now', 'localtime')`;
    const dTimeCondition = endTime ? `AND d.fecha >= $2 AND d.fecha <= $3` : `AND d.fecha >= $2 AND d.date(fecha) <= date('now', 'localtime')`;
    ...
    const ventas = await pool.query(
      ...
      const gastosTimeCondition = endTime 
        ...
        : `AND fecha >= $2 AND date(fecha) <= date('now', 'localtime')`;
