...
js = js.replace('li.textContent = `${c.nombre} (${c.documento || \'-\'})`;', 'li.textContent = c.documento ? `${c.nombre} (${c.documento})` : c.nombre;');
...
const newProdSearch = `const filtrados = productosCache.filter((p) => p.nombre.toLowerCase().includes(texto) || (p.codigo_barras && p.codigo_barras.includes(texto)));`;
