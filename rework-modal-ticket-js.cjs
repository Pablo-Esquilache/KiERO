...
const oldFuncRegex = /document\.querySelectorAll\("\.btn-ver-ticket:not\(\.btn-ver-devolucion\)"\)\.forEach\(\(btn\) => \{[\s\S]*?modalTicket\.style\.display = "flex";\s*\}\);\s*\}\);/g;

const newFuncStr = `document.querySelectorAll(".btn-ver-ticket:not(.btn-ver-devolucion)").forEach((btn) => {
...
if (js.match(oldFuncRegex)) {
  js = js.replace(oldFuncRegex, newFuncStr);
  
  // also wire up the print button for this new modal
  const addPrintBtn = `
  ...
  console.log("Updated JS for modalTicket");
} else {
  console.log("Could not match JS logic for modalTicket");
}
